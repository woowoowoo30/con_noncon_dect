...
class Detector(object):
    ...
    def plot_bboxes(self, image, bboxes, line_thickness=None):
        ...
        for (x1, y1, x2, y2, cls_id, conf) in bboxes:
            ...
            # t_size = cv2.getTextSize(self.names[cls_id], 0, fontScale=tl / 3, thickness=tf)[0]
            t_size = cv2.getTextSize(f'{cls_id}', 0, fontScale=tl / 3, thickness=tf)[0]
        ...
        for det in pred:
            if det is not None and len(det):
                ...
                for *x, conf, cls_id in det:
                    ...
                    b = (float(x[0]), float(x[2]), float(x[1]), float(x[3]))
                    bb = self.convert((w,h), b)

                    # image_info[key] = [f'{x1} {x2} {y1} {y2} {lbl}', np.round(float(conf), 3)]
                    image_info.append(f'{lbl} {" ".join([str(a) for a in bb])} {np.round(float(conf), 3)}')
